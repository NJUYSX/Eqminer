...
void Judgement::GenerateRandomCase() {
    ...
    for (int i = 0; i < stdin_vector.size(); i++) {
        File << random_case(i) << ' ';
    }
    File << endl;
    File.close();
...
void Judgement::Judge() {
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        //1.out <input.txt >output1.txt #��1.out����Ŀ¼��ִ�и�����
        ...
        //2.out <input.txt >output2.txt #��2.out����Ŀ¼��ִ�и�����
...
void runJudgement() {
    for (int i = 0; i < input_vector.size() - 1; ++i) {
        for (int j = i + 1; j < input_vector.size(); ++j) {
            ...
            Output* temp_output = new Output(make_pair(input_vector[i]->get_relative_path(), input_vector[j]->get_relative_path()),false);
            output_vector.push_back(temp_output);

            //generate judgement objects
            Judgement* judgement = new Judgement(input_vector[i], input_vector[j], temp_output, stdin_vector);
